function handleResult(resultData) {
    ...
    resultData.forEach((genre, index) => {
        let link = `<a href="browse.html?genre=${encodeURIComponent(genre)}" class="text-center">${genre}</a>`;
...
function showPrefixes() {
    ...
    for (const prefix of prefixes) {
        let link = `<a href="browse.html?prefix=${encodeURIComponent(prefix)}">${prefix}</a>`;
